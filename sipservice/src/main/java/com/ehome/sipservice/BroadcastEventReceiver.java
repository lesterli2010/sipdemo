...
import com.ehome.sipservice.BroadcastEventEmitter.BroadcastParameters;
...
import static com.ehome.sipservice.BroadcastEventEmitter.BroadcastAction.CALL_STATE;
import static com.ehome.sipservice.BroadcastEventEmitter.BroadcastAction.CODEC_PRIORITIES;
import static com.ehome.sipservice.BroadcastEventEmitter.BroadcastAction.CODEC_PRIORITIES_SET_STATUS;
import static com.ehome.sipservice.BroadcastEventEmitter.BroadcastAction.DTMF;
import static com.ehome.sipservice.BroadcastEventEmitter.BroadcastAction.INCOMING_CALL;
import static com.ehome.sipservice.BroadcastEventEmitter.BroadcastAction.OUTGOING_CALL;
import static com.ehome.sipservice.BroadcastEventEmitter.BroadcastAction.REGISTRATION;
import static com.ehome.sipservice.BroadcastEventEmitter.BroadcastAction.STACK_STATUS;
...
public class BroadcastEventReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(BroadcastEventEmitter.getAction(REGISTRATION))) {
            ...
            int statusCode = intent.getIntExtra(BroadcastParameters.STATUS_CODE, 0);
            pjsip_status_code sc = null;
            try {
                if (statusCode != 0) {
                    sc = pjsip_status_code.swigToEnum(statusCode);
                }
            } catch (Exception e){}

            onCallState(intent.getStringExtra(BroadcastParameters.ACCOUNT_ID),
                        intent.getIntExtra(BroadcastParameters.CALL_ID, -1),
                        pjsip_inv_state.swigToEnum(callState), sc,
        ...
        } else if (action.equals(BroadcastEventEmitter.getAction(DTMF))) {
            onReceiveDtmf(intent.getStringExtra(BroadcastParameters.ACCOUNT_ID),
                    intent.getIntExtra(BroadcastParameters.CALL_ID, -1),
                    intent.getStringExtra(BroadcastParameters.DTMF));
        }
    }
    ...
    public void register(final Context context) {
        ...
        intentFilter.addAction(BroadcastEventEmitter.getAction(DTMF));
    ...
    public void onCallState(String accountID, int callID, pjsip_inv_state callStateCode, pjsip_status_code statusCode,
    ...
    public void onReceiveDtmf(String accountID, int callID, String dtmfDigit) {
        Logger.debug(LOG_TAG, "Receive DTMF  - accountID: " + accountID +
                ", callID: " + callID + ", dtmfDigit:" + dtmfDigit);
