...
public class SipCall extends Call {
    ...
    public void onCallState(OnCallStateParam prm) {
        try {
            ...
            Logger.debug(LOG_TAG, "onCallState: callState:" + callState + ", statusCode:" + (statusCode == null ? "null" : statusCode)
            + ", role:" + info.getRole());
            ...
            if (callState == pjsip_inv_state.PJSIP_INV_STATE_DISCONNECTED) {
                ...
                // check if (180 or 183) && call is outgoing (ROLE UAC)
            ...
            account.getService().getBroadcastEmitter()
                    .callState(account.getData().getIdUri(), callID, callState.swigValue(), statusCode != null ? statusCode.swigValue() : 0,
    ...
    public void onDtmfDigit(OnDtmfDigitParam prm) {
        try {
            ...
            account.getService().getBroadcastEmitter()
                    .incomingDtmf(account.getData().getIdUri(), callID, prm.getDigit());
        } catch (Exception e) {
            Logger.error(LOG_TAG, "Error while receive DTMF", e);
        }
    }
